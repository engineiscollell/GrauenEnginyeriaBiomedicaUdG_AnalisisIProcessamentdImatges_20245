...
net = alexnet;
featureLayer = 'fc7'; % extreurem característiques del layer fc8
...
sgtitle('Corbes ROC per classe (AlexNet, fc7, SVM, Distància Euclidiana)');
...
xtickangle(45);
ylabel('AUC');
title('AUC per classe (AlexNet, fc7, SVM, Distància Euclidiana)');
grid on;
...
function classifier = train(VOCopts,cls, net, featureLayer)
   ...
   % Entrenem classificador SVM
    classifier.model = fitcknn(classifier.FD', classifier.gt', 'NumNeighbors', 3); % canviar k
...
function c = classify(classifier, fd)
    ...
    [~, score] = predict(classifier.model, fd); % predicció amb kNN
